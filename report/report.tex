...
Hyper-edges are coarsened natively at every level; the structural clustering signal
uses arity-normalised co-membership ($|M(e)|^{-1}$) rather than flat pairwise expansion.
...
\textbf{P4} hyper-edge fidelity — hyper-edges are coarsened as single units at
every level; the structural affinity uses arity-normalised co-membership
($|M(e)|^{-1}$, not flat clique expansion).
...
\begin{equation}
  ...
  \label{eq:affinity}
\end{equation}
where $\phi$ are MiniLM sentence embeddings and the second term is our
$1/|M(e)|$ arity-normalised co-occurrence (normalised to $[0,1]$; note this
differs from Chodrow et al.'s $1/(|e|-1)$ — a deliberate design choice to
dampen large edges rather than preserve node degree).
\end{problem}
...
The T4 collapse rule holds by construction (\texttt{hyperedge\_collapse.py} coarsens
hyper-edges as single units at every level).
The structural affinity in Eq.~\ref{eq:affinity} uses arity-normalised
co-membership ($|M(e)|^{-1}$), which dampens large edges without eliminating
their pairwise co-occurrence signal; it is not a full clique expansion.
...
Chodrow et al.~\cite{chodrow2021hypergraph} develop a degree-corrected stochastic
block model and modularity objective for hypergraphs with heterogeneous edge sizes,
showing edge-size-aware treatment outperforms dyadic projections.
Inspired by this, we use a $1/|M(e)|$ co-membership signal combined with semantic
similarity (our design; their clique-projection baseline uses $1/(|e|-1)$).
...
each level's coarsened hypergraph is stored as a structural output alongside the
partition.  These coarse edges are available for downstream analysis at any
resolution.

\subsection{Temporal Coupling (T3)}\label{sec:t3}

We build the hierarchy independently at each snapshot with a warm-start prior:
the level-0 cluster assignments from the previous snapshot are passed as
\texttt{warm\_labels} to the hierarchy builder, which adds a small affinity bonus
($+0.05$) between node pairs that shared a cluster in the preceding snapshot.
This biases the Ward dendrogram toward the previous solution in stable regions
without hard-constraining it.

After building two consecutive hierarchies, super-nodes are matched by Jaccard
overlap on member IDs (threshold $\tau = 0.30$).
The matching is many-to-many, enabling detection of merge and split events in
addition to the standard birth/growth/shrink/death lifecycle.
A match is classified as a \emph{merge} when multiple previous super-nodes all
have Jaccard $\ge \tau$ with the same current super-node; a \emph{split} when
one previous super-node maps to two or more current super-nodes.
...
\bottomrule
\end{tabular}
\caption{327 events across three transitions (levels 0 and 1), including
merge events detectable via the many-to-many Jaccard matching scheme.
The high birth/death count reflects the +90\% node surge in 2022$\to$2024,
not instability --- cross-snapshot ARI with 95\% CIs in §\ref{sec:eval} confirms this.}
\label{tab:events}
\end{table}
...
ARI between consecutive snapshots (shared nodes only) with 95\% bootstrap CIs
($n=500$ resamples) —
level~0: $0.29\,[0.27,\,0.31]\ \to\ 0.30\,[0.28,\,0.32]\ \to\ 0.36\,[0.34,\,0.38]$;
level~1: $0.41\,[0.39,\,0.47]\ \to\ 0.33\,[0.32,\,0.37]\ \to\ 0.38\,[0.37,\,0.41]$.
These are moderate values.
The 2022$\to$2024 transition adds +90\% new nodes, so genuine reorganisation is
expected — the event log records 111 births and 2 merges confirming real change,
not noise.
The CIs are narrow (width $\approx$0.03--0.07) because the shared-node set is large
(1,527--4,184 nodes), giving stable estimates.
...
Table~\ref{tab:verified} distinguishes claims we verified empirically or
programmatically from those we left as stated assumptions.
Flagging assumptions explicitly is a deliberate choice: it makes the
limits of the work visible and prevents over-reading the results.
...
Cross-snapshot ARI has bootstrap 95\% CIs & \textbf{Verified} — 500 bootstrap resamples \\
Warm-start biases affinity toward prior partition & \textbf{Verified} — \texttt{warm\_labels} bonus implemented \\
Merge/split events are detectable & \textbf{Verified} — many-to-many Jaccard matching; 2 merges found \\
